...
PhysicsEngineClass = Class.extend({
    ...
    update:function () {
        var start = Date.now();
        this.world.Step(Constants.PHYSICS_LOOP_HZ, //frame-rate
    ...
    addBody:function (entityDef) {
        ...
        if (entityDef.linearDamping) {
            bodyDef.linearDamping = entityDef.linearDamping;
        }

        if (entityDef.angularDamping) {
            bodyDef.angularDamping  = entityDef.angularDamping;
        }

        var body = this.registerBody(bodyDef);

        var lMassData = new MassData();
        lMassData.mass = 0.4;
        ...
        fixtureDefinition.friction = (entityDef.friction) ? entityDef.friction : 0; //0.5;//0.0;
        fixtureDefinition.restitution = 0.3; //0.2;
